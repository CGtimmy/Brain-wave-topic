...
for i = 1:length(exceed_idx)
    ...
    elseif idx == length(avg_power_10_60)  % 最後一個點，只能用前一個點的值
        avg_power_10_60(idx) = avg_pwer_10_60(idx - 1);
    else
...
for i = 1:length(exceed_idx2)
    idx = exceed_idx2(i);
    if idx == 1  % 第一個點，只能用後一個點的值
        avg_power_230_280(idx) = avg_power_230_280(idx + 1);
    elseif idx == length(avg_power_230_280)  % 最後一個點，只能用前一個點的值
        avg_power_230_280(idx) = avg_power_230_280(idx - 1);
    else
        avg_power_230_280(idx) = mean([avg_power_230_280(idx - 1), avg_power_230_280(idx + 1)]);
    end
end
...
xlabel('時間段');
ylabel('平均功率');
set(gca, 'XTickLabel', {'10-60s', '230-280s'});
grid on;
